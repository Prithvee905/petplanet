        ...
        <div className="grid grid-cols-1 lg:grid-cols-[0.95fr_1.05fr] gap-12 lg:gap-16 items-start">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 auto-rows-[240px] md:auto-rows-[220px]">
            {diagnosticsShowcase.map((item, index) => (
              <motion.div
                key={item.title}
                className={`group relative overflow-hidden rounded-[28px] border border-white/10 bg-black shadow-[0_24px_80px_rgba(0,0,0,0.35)] ${item.className}`}
              ...
              >
                <img
                  src={item.image}
                  alt={item.title}
                  className={`h-full w-full ${item.imageClassName} object-contain transition-transform duration-700 group-hover:scale-105`}
                />
